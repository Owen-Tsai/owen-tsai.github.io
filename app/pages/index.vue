...
const sketch = (p: P5) => {
  ...
  const createText = () => {
    const text = texts[p.floor(p.random(texts.length))]
  ...
  p.windowResized = () => {
    p.resizeCanvas(Math.min(p.windowWidth, 1440), p.windowHeight)
    textBox.placeOnCanvas()
    textBox.restore()
  }

  p.setup = async () => {
    p.createCanvas(Math.min(p.windowWidth, 1440), p.windowHeight)
    mouse = new Mouse(100)
    createText()
  ...
  p.draw = () => {
    ...
    if (isMobile.value) {
      ...
      p.text(
        'Hover over or press D for random distortion',
        textBox.boundingBox.x + 10,
  ...
  class Point {
    ...
    update() {
      ...
      if (distanceSquared < mouse.radiusSquared) {
        ...
        // 按照距离鼠标的远近程度赋予扭曲力度因子
        const factor = p.map(distance, 0, mouse.radius, 0.8, 0)
        // 根据鼠标速度添加位移
